...
export function NameExplorer({ names, initialQuery }: NameExplorerProps) {
  ...
  const [vibe, setVibe] = useState<NameQuery["vibe"]>(initialQuery.vibe);
  const [breed, setBreed] = useState<NameQuery["breed"]>(initialQuery.breed);
  ...
  const matches = useMemo(() => filterNames(names, query), [names, query]);
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {shortlist.length === 0
                ? "Heart names as you browse. Copy the list when you are ready to vote."
        ...
        {shortlist.length === 0 ? (
          <div className="mt-4 rounded-2xl border border-dashed border-border/80 bg-card/60 px-4 py-8 text-center">
            <p className="font-heading text-lg tracking-tight">Nothing saved yet</p>
            <p className="mx-auto mt-1 max-w-sm text-sm text-muted-foreground">
              Tap the heart on any card. Your picks stay here until you clear them.
            </p>
            <Button type="button" className="mt-4 rounded-full" asChild>
              <a href="#finder">Start with breed and vibe</a>
            </Button>
          </div>
        ) : (
      ...
      <div id="names" className="flex scroll-mt-24 flex-col gap-8">
        <div className="rounded-3xl border border-border/80 bg-card/85 p-4 shadow-[0_18px_40px_-28px_oklch(0.35_0.05_50/0.45)] sm:p-6">
          ...
          <div className="mt-6 grid gap-5">
            ...
            <FilterRow label="Tag">
              <FilterChip pressed={vibe === "all"} onClick={() => setVibe("all")}>
                All
              </FilterChip>
              {VIBE_FILTERS.map((value) => (
                <FilterChip
                  key={value}
                  pressed={vibe === value}
        ...
        {matches.length === 0 ? (
          <div className="rounded-3xl border border-dashed bg-card px-5 py-14 text-center">
            ...
            <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-muted-foreground">
              Try a broader breed, clear the vibe chip, or search a shorter word.
              Meanings work too, like moon or snow.
            </p>
